...
def plot_test3(member_name):
    ...
    if not files:
        ...
        return
    if not HAS_MPL:
        print("[INFO] Skipping Test 3 matplotlib rendering (requires pandas/matplotlib)")
        return
...
def plot_test4(member_name):
    ...
    if HAS_MPL:
        ...
        ax.set_xlabel('Target Degree Limit ($k$)')
        ax.set_ylabel('Concurrent Burst Size ($s$)')
        ax.set_title(r'Test 4: Bubble Chart of $k$ vs $s$ (Initial $N=10\mathrm{M}$, Size = Trials)')
        ...
        ax.set_xlabel('Concurrent Burst Size ($s$)')
        ax.set_ylabel('Success Rate (%)')
        ax.set_title(r'Test 4: Success Rate vs. Concurrent Burst Size ($s$, Initial $N=10\mathrm{M}$)')
        ax.set_ylim(-5, 115)
        ax.grid(True, linestyle='--', alpha=0.7)
...
\begin{axis}[
    ...
    title={Test 4: Bubble Chart of $k$ vs $s$ (Initial $N=10\mathrm{M}$, Size = Trials)},
    xmin=3, xmax=11,
    ymin=0, ymax=32,
    xtick={4,6,8,10},
...
\begin{axis}[
    ...
    ylabel={Success Rate (\%)},
    title={Test 4: Success Rate vs. Concurrent Burst Size ($s$, Initial $N=10\mathrm{M}$)},
    ymin=-5, ymax=115,
    xmin=0, xmax=31,
    grid=both,
