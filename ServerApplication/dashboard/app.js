...
async function fetchFailureReports() {
    ...
    if (!response.ok) {
        throw new Error(`GET /api/failure-reports returned HTTP ${response.status}`);
    }
...
function updateCards(nodes, failureReports) {
    const upNodes = nodes.filter((node) => normalizeStatus(node.status) === "UP").length;

    const failedNodes = nodes.filter((node) => {
        const status = normalizeStatus(node.status);
        return status === "FAILED" || status === "DOWN" || status === "UNREACHABLE";
    }).length;
...
function updateAlert(nodes, failureReports) {
    ...
    for (const node of nodes) {
        ...
        if (status === "FAILED" || status === "DOWN" || status === "UNREACHABLE") {
...
async function loadHistory(nodeId) {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`GET /api/nodes/${nodeId}/history returned HTTP ${response.status}`);
        }
...
function statusClass(status) {
    ...
    if (normalized === "DOWN" || normalized === "FAILED" || normalized === "UNREACHABLE") {
        return "status-down";
    }
