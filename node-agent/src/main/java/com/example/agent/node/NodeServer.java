...
class NodeServer {
    ...
    NodeServer(
        ...
        // This is the decentralized replacement for sending failure reports to a
        // central server.
    ...
    private void handleGossip(HttpExchange exchange) throws IOException {
        ...
        String senderNodeId = P2pJson.stringValue(requestBody, "senderNodeId");
    ...
    private void handleJoin(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            String responseJson = joinAckJson(true, suggestedPeer.orElse(null),
                    "candidate accepted; confirmation required");
            sendResponse(exchange, 200, responseJson);
    ...
    private void handleJoinConfirm(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            if (result.evictedPeer() != null) {
                nodeClient.notifyNeighborRemove(result.evictedPeer(), localAddress);
            }
    ...
    private void handleNeighborRemove(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            sendResponse(exchange, 400,
                    "{\"type\":\"ERROR\",\"message\":\"" + P2pJson.escape(exception.getMessage()) + "\"}");
        }
    }
    ...
    private void handleFailureEvent(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            if (added) {
                ...
                // Duplicate events are ignored by FailureEventLog.add(...), and TTL prevents
                // endless loops.
    ...
    private void forwardReceivedFailureEvent(FailureEvent event) {
        ...
        for (NodeAddress neighbor : neighborDirectory.addresses()) {
            ...
            nodeClient.sendFailureEvent(neighbor, forwardedEvent)
                    .thenAccept(sent -> {
                        if (sent) {
                            System.out.println(
                                    "[" + LocalDateTime.now() + "] "
